...
class Logger:
    ...
    def visualize_rec(self, inp, out, name):
        if name is not None: 
            image = self.visualizer.visualize_dual_reconstruction(inp, out)
        else:
            image = self.visualizer.visualize_reconstruction(inp, out) 
            name = 'yolo' 

        imageio.mimsave(os.path.join(self.visualizations_dir, "{}-{}-rec.gif".format(name, 
            str(self.it).zfill(self.zfill_num))), image)
    ...
    def log_iter(self, it, names, values, inp, out, name):
        self.it = it
        self.names = names
        self.loss_list.append(values)
        if it % self.log_freq == 0:
            self.log_scores(self.names)
            self.visualize_rec(inp, out, name)
...
class Visualizer:
    ...
    def draw_video_with_kp(self, video, kp_array):
        ...
        for i in range(len(video_array)):
            for kp_ind, kp in enumerate(kp_array[i]):
                ...
                # print(video_array[i][rr,cc].shape)
    ...
    def visualize_dual_reconstruction(self, inp, out):
        out_video_batch = out['video_prediction'].data.cpu().numpy()
        if 'driving' in inp:
            gt_video_batch = inp['driving'].data.cpu().numpy()
        else:
            driving_video_batch = inp['video'].data.cpu().numpy()
            gt_video_batch = inp['gt_video'].data.cpu().numpy() 

        appearance_deformed_batch = out['video_deformed'].data.cpu().numpy()
        appearance_video_batch = inp['source'].data.cpu().repeat(1, 1, 
                out_video_batch.shape[2], 1, 1).numpy()

        kp_video = out['kp_driving']['mean'].data.cpu().numpy()
        kp_gt_video = out['gt_kp_driving']['mean'].data.cpu().numpy() if 'gt_kp_driving' in out else None
        kp_appearance = out['kp_source']['mean'].data.cpu().repeat(1, 
                out_video_batch.shape[2], 1, 1).numpy()

        out_video_batch = np.transpose(out_video_batch, [0, 2, 3, 4, 1])
        driving_video_batch = np.transpose(driving_video_batch, [0, 2, 3, 4, 1]) 
        gt_video_batch = np.transpose(gt_video_batch, [0, 2, 3, 4, 1])
        appearance_video_batch = np.transpose(appearance_video_batch, [0, 2, 3, 4, 1])
        appearance_deformed_batch = np.transpose(appearance_deformed_batch, [0, 2, 3, 4, 1])

        if kp_gt_video is None:
            image = self.create_image_grid((appearance_video_batch, kp_appearance), 
                (driving_video_batch, kp_video), (out_video_batch, kp_video) ,appearance_deformed_batch, 
                (gt_video_batch, kp_video))
        else:
            image = self.create_image_grid((appearance_video_batch, kp_appearance), 
                (driving_video_batch, kp_video), (out_video_batch, kp_video) ,appearance_deformed_batch, 
                (gt_video_batch, kp_gt_video))
    ...
    This is for visualizing the pose for the first frame of a GIF file (provided as the source) 

    '''
    def visualize_initial_pose(self, source_image, keypoints, spatial_size=128):
        
        kp_appearance = keypoints 
        appearance_video_batch = source_image.data.cpu().numpy()

        # image = self.create_image_grid((appearance_video_batch, kp_appearance))
       
        kp_array = kp_appearance.squeeze()
        # print(kp_array.shape) 
        num_kp = kp_array.shape[0]
        kp_array = kp_array.reshape((-1, num_kp, self.kp_size)) # 1, 10, 2
        # print(kp_array.shape, num_kp)

        appearance_video_batch = appearance_video_batch.reshape((-1, spatial_size, spatial_size, 3)) 
        # print(appearance_video_batch.shape)
        ...
        for kp_ind, kp in enumerate(kp_array[0]):
            # print(kp[1], kp[0]) 
            rr, cc = circle(kp[1], kp[0], self.kp_size, shape=(spatial_size, spatial_size))
            # print(rr, cc)
            # print(np.array(self.colormap(kp_ind / num_kp)))
            appearance_video_batch[0][rr, cc] = np.array(self.colormap(kp_ind / num_kp))[:3]
        image = appearance_video_batch 
        image = (255 * image).astype(np.uint8)
        return image 
